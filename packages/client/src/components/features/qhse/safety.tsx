          ...
          {images.map((image) => (
            <div key={image.title} className="relative h-[160px] rounded-[2px] overflow-hidden">
              ...
              <div className="absolute left-3 bottom-3 text-left">
                <div className="text-[9px] uppercase leading-3 text-secondary font-bold">{image.title}</div>
                <div className="text-[13px] font-bold text-white capitalize leading-5">{image.description}</div>
              </div>
            </div>
          ))}
        </div>
