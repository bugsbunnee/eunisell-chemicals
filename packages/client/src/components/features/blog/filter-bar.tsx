...
import { SearchIcon, ChevronDownIcon, SlidersHorizontalIcon } from 'lucide-react';
...
const BlogFilterBar: React.FC = () => {
  ...
  return (
    <div className="sticky top-0 z-20 backdrop-blur-[6px] bg-white/95 border-y border-border px-6 py-4 md:px-30 md:py-8 text-left">
      ...
      <div className="relative w-full md:hidden mb-4">
        ...
        <input
          ...
          className="w-full bg-card border border-border pl-11 pr-4 py-3.5 text-sm text-accent placeholder:text-[#9ca3af] outline-none focus:border-secondary/40 transition-colors rounded-sm"
        />
      </div>
            ...
            <div className="absolute top-full right-0 mt-2 bg-white border border-border shadow-md z-30 min-w-37.5">
              {sortOptions.map((opt) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm hover:bg-card transition-colors ${opt === sortBy ? 'font-semibold text-accent' : 'text-[#777]'}`}
                >
                  {opt}
                </button>
              ))}
            ...
            <div className="absolute top-full left-0 mt-2 bg-white border border-border shadow-md z-30 min-w-[180px]">
              {topics.map((t) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm hover:bg-card transition-colors ${t === activeTopic ? 'font-semibold text-accent' : 'text-[#777]'}`}
                >
                  {t}
                </button>
              ))}
        ...
        <div className="flex items-center gap-4 shrink-0">
          <div className="relative w-75">
            ...
            <input
              ...
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full bg-card border border-border pl-11 pr-4 py-3 text-sm text-accent placeholder:text-[#9ca3af] outline-none focus:border-secondary/40 transition-colors"
            />
          </div>
