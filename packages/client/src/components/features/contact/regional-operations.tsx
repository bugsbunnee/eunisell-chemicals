...
const RegionalOperations: React.FC = () => {
  return (
    <section className="px-6 py-16 md:px-30 md:py-24 bg-card border-t border-border">
      {/* Eyebrow */}
      ...
      <div className="mt-10 md:mt-12 flex flex-col md:grid md:grid-cols-3 md:gap-x-12 gap-8">
        {operations.map((operation) => (
          <div key={operation.title} className="text-left">
            <div className="h-[200px] md:h-64 w-full overflow-hidden rounded-[4px] relative">
              <img src={operation.src} alt={operation.title} className="h-full w-full object-cover" />
              <div className="absolute top-3 left-3 bg-accent/80 text-white text-[10px] font-bold uppercase tracking-[1.5px] px-3 py-1 rounded-[2px]">
                {operation.badge}
              </div>
            </div>
            ...
            <div className="flex items-start gap-x-2 text-base leading-6.5 text-primary font-normal">
              <MapPinIcon size={16} className="text-secondary shrink-0 mt-0.5" />
