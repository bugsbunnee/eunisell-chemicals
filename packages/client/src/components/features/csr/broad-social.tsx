...
const cards = [
  {
    ...
    mobileDesc: 'Encouraging physical activity and wellness across all age groups.',
...
const BroadSocial: React.FC = () => {
  return (
    <section className="bg-card text-left">
      {/* Mobile layout — show first 3 cards stacked */}
      <div className="md:hidden px-6 py-16 flex flex-col gap-8">
        <h2 className="font-bold text-[24px] leading-[30px] text-accent text-center">Broad-Scale Social Value</h2>

        <div className="flex flex-col gap-4">
          {cards.slice(0, 3).map(({ title, mobileDesc }) => (
            <div
              key={title}
              className="bg-white border border-border drop-shadow-[0px_1px_1px_rgba(0,0,0,0.05)] flex flex-col gap-2 p-6 rounded-[4px]"
            >
              <div className="bg-secondary rounded-full size-3 shrink-0" />
              <h6 className="font-bold text-[18px] leading-6 text-accent">{title}</h6>
              <p className="text-[14px] leading-[22.75px] text-card-foreground">{mobileDesc}</p>
            </div>
          ))}
        </div>
      </div>
