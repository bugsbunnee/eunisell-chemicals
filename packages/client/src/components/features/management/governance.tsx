...
const Governance: React.FC = () => {
  ...
  return (
    <section className="flex flex-col md:grid md:grid-cols-2 md:gap-x-24 py-16 px-6 md:py-24 md:px-30 bg-background text-left gap-8">
      <div>
        <div className="text-[32px] md:text-5xl font-bold text-accent leading-10 md:leading-15">Strategic Governance</div>

        <p className="text-[16px] md:text-lg leading-6.5 md:leading-[29.3px] text-card-foreground font-normal mt-6 md:mt-8">
          Our management framework facilitates quick decision-making while maintaining accountability.
        </p>

        <p className="text-[16px] md:text-lg leading-6.5 md:leading-[29.3px] text-card-foreground font-normal mt-4 md:mt-6">
        ...
        <div className="flex flex-col gap-4 mt-8">
          {policies.map((policy) => (
            <div key={policy.title} className={cn('p-5 bg-card flex items-center gap-x-4 rounded-[2px] border-l-4', policy.className)}>
              <div>
                <div className="text-sm md:text-base leading-5.25 md:leading-7.5 text-accent font-semibold">{policy.title}</div>
                <div className="text-[13px] md:text-[15px] leading-5.25 md:leading-[22.5px] text-card-foreground font-normal">{policy.description}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="hidden md:flex flex-col space-y-6">
        ...
        <p className="text-lg leading-[29.3px] text-card-foreground font-normal">
          Our management framework is designed to facilitate quick decision-making while maintaining the highest levels of accountability. We believe that robust governance is the
          foundation of industrial trust.
        </p>

        <p className="text-lg leading-[29.3px] text-card-foreground font-normal">
          Eunisell's executive team works closely with technical directors to ensure that every investment and operational move aligns with our core mission.
        </p>
      </div>
    </section>
  );
