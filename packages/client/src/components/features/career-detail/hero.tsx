...
const CareerDetailHero: React.FC<CareerDetailHeroProps> = ({ title, department, location, type, level, deadline, isDeadlineSoon = true, onApply }) => (
  <section className="bg-card border-b border-border text-left relative overflow-hidden py-16 px-30">
    ...
    <div className="relative max-w-[1680px]">
      ...
      <p className="text-muted-foreground text-[20px] leading-[1.625] max-w-200 mb-8">
        Join our technical team to drive growth and deliver high-performance chemical solutions to major industrial and oil &amp; gas clients across West Africa.
      </p>
      ...
      <div className="flex items-center gap-4">
        <button
          onClick={onApply}
          className="bg-secondary text-white font-bold text-[16px] px-10 h-14 rounded-[4px] shadow-[0px_10px_15px_-3px_rgba(0,131,206,0.2),0px_4px_6px_-4px_rgba(0,131,206,0.2)] hover:bg-secondary/90 transition-all hover:scale-105"
        >
          Apply for this Role
        </button>
        <Link
          to={paths.careers}
          className="flex items-center justify-center border-2 border-accent text-accent font-bold text-[16px] px-8 h-14 rounded-[4px] hover:bg-accent/5 transition-all hover:scale-105"
        >
          View All Openings
        </Link>
      </div>
